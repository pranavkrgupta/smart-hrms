  ...
  return (
    <div className="flex flex-col w-full p-6 bg-white shadow-md rounded-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-8 text-sm md:text-base mb-6">
        <p><span className="font-semibold">Total Employees</span> : {dashboardData.totalEmployees}</p>
        <p><span className="font-semibold">Total Departments</span> : {dashboardData.totalDepartments}</p>
        <p><span className="font-semibold">Total Designation</span> : {dashboardData.totalDesignations}</p>
        <p>
          <span className="font-semibold">Today's Attendance</span> : {dashboardData.todayAttendance.present} Present / {dashboardData.todayAttendance.absent} Absent
        </p>
        <p><span className="font-semibold">Pending Leave Request</span> : {dashboardData.pendingLeaveRequests}</p>
      </div>
